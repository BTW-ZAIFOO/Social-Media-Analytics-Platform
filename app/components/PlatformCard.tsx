...
import { PlatformIcon, getPlatformColor, getPlatformGradient } from './PlatformIcon';
...
export default function PlatformCard({ metrics, onSelect, onDelete }: PlatformCardProps) {
    ...
    >
      ...
      <div className="relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">
                {metrics.platform.charAt(0).toUpperCase() + metrics.platform.slice(1)}
              </h3>
              <p className="text-xs text-slate-600">{metrics.accounts} account(s)</p>
            </div>
          </div>
          {onDelete && (
            <button
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Followers</p>
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Engagement</p>
            <p className="mt-1 flex items-baseline gap-1">
              <span className="text-2xl font-bold text-slate-900">{metrics.totalEngagement.toFixed(1)}</span>
        ...
        <div className="mt-4 flex items-center gap-2 rounded-2xl bg-white/50 px-3 py-2">
          ...
          <span className={`text-sm font-semibold ${metrics.growth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {metrics.growth >= 0 ? '+' : ''}{metrics.growth.toFixed(1)}% growth
          </span>
        </div>
      </div>
    </div>
