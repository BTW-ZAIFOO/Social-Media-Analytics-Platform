import type { FacebookPostData, SocialMediaStats, SocialMediaPost } from '../types/social-api.ts';

const mockFacebookPosts: FacebookPostData[] = [
  {
    id: 'post_fb_001',
...
export async function fetchFacebookData(profileUrl: string, accessToken?: string): Promise<SocialMediaStats> {
  try {
    ...
    return {
      ...
      engagement: posts.length > 0 ? posts.reduce((sum, p) => sum + p.engagementRate, 0) / posts.length : 0,
...
export async function fetchFacebookRealtime(profileUrl: string, accessToken?: string): Promise<FacebookPostData[]> {
  try {
    if (!accessToken) {
    ...
    const fields = ['id', 'message', 'created_time', 'full_picture', 'shares', 'comments.summary(true)', 'likes.summary(true)'].join(',');
