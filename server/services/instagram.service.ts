import type { InstagramPostData, SocialMediaStats, SocialMediaPost } from '../types/social-api.ts';
...
export async function fetchInstagramData(profileUrl: string, accessToken?: string): Promise<SocialMediaStats> {
  try {
    ...
    const posts = rawPosts.map((post): SocialMediaPost => {
      ...
      return {
        ...
        shares: 0,
        views: post.media_type === 'VIDEO' ? (post.like_count ?? 0) * 5 : (post.like_count ?? 0) * 3,
...
export async function fetchInstagramRealtime(profileUrl: string, accessToken?: string): Promise<InstagramPostData[]> {
  try {
    if (!accessToken) {
    ...
    return data.data.map((item: any) => ({
      ...
      video_url: item.media_url && item.media_type === 'VIDEO' ? item.media_url : undefined,
