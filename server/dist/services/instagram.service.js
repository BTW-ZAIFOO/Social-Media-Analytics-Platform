...
exports.fetchInstagramRealtime = fetchInstagramRealtime;
...
async function fetchInstagramData(profileUrl, accessToken) {
    try {
        console.log(`Fetching Instagram data for: ${profileUrl}`);
        if (!accessToken) {
            throw new Error("Instagram access token not provided. Real profile data cannot be fetched without a valid Instagram API access token.");
        }
...
async function fetchInstagramRealtime(profileUrl, accessToken) {
    try {
        if (!accessToken) {
            throw new Error("Instagram access token is required to fetch live post data.");
        }
        const url = `https://graph.instagram.com/me/media?fields=id,caption,media_type,media_url,thumbnail_url,timestamp,like_count,comments_count&access_token=${encodeURIComponent(accessToken)}`;
        ...
        return data.data.map((item) => ({
            ...
            video_url: item.media_url && item.media_type === "VIDEO"
                ? item.media_url
                : undefined,
